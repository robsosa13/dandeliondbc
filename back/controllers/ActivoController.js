...
function editar(req,res){
    ...
    if(req.files.imagen){
        ...
        Activo.findByIdAndUpdate({_id:id},{titulo: data.titulo, descripcion: data.descripcion, imagen:imagen_name, precio_compra: data.precio_compra, puntos: data.puntos, idcategoria: data.idcategoria}, (err, activo_edit) =>{
            if(err){
                res.status(500).send({message: 'Error en el servidor'});
            }else{
                if(activo_edit){
        ...
        Activo.findByIdAndUpdate({_id:id},{titulo: data.titulo, descripcion: data.descripcion, precio_compra: data.precio_compra,pertenece: data.pertenece, idcategoria: data.idcategoria}, (err, activo_edit) =>{
            if(err){
                res.status(500).send({message: 'Error en el servidor'});
            }else{
                if(activo_edit){
